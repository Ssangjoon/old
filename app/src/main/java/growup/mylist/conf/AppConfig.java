...
//    이름과 관계없이 애노테이션이 붙어있다면 전부 생성하기 때문에 이름을 혼용해도 
//    사용에는 지장이 없지만, 약속이니까 객체에 맞는 애노테이션을 붙여야 한다. 
@ComponentScan("growup.mylist") //AnnotationConfigWebApplicationContext.register()(에 잘 짜여져 있음)

//2) DB관련 설정을 할 때 DBMS 정보를 담은 properties 파일을 로딩한다.
@PropertySource("classpath:growup/mylist/conf/jdbc.properties")
...
public class AppConfig { 
  ...
  public SqlSessionFactory sqlSessionFactory(
    ...
    sqlSessionFactoryBean.setTypeAliasesPackage("growup.mylist.domain");
    sqlSessionFactoryBean.setMapperLocations(
        appCtx.getResources("classpath:growup/mylist/dao/*Dao.xml"));
